...
int main(int argc, char *argv[])
{
	...
	auto heights_base = gli::load(argv[1]);
	if (heights_base.empty())
	{
		LOGE("Failed to load height: %s\n", argv[1]);
		return 1;
	}
	...
	if (heights_base.format() == gli::FORMAT_R32_SFLOAT_PACK32)
	{
		const float *buffer = static_cast<const float *>(heights_base.data());
		for (int y = 0; y < height; y++)
			for (int x = 0; x < width; x++)
				data[y * width + x] = buffer[y * width + x];
	}
	else if (heights_base.format() == gli::FORMAT_R16_SFLOAT_PACK16)
	{
		const uint16_t *buffer = static_cast<const uint16_t *>(heights_base.data());
		for (int y = 0; y < height; y++)
			for (int x = 0; x < width; x++)
				data[y * width + x] = unpackHalf1x16(buffer[y * width + x]);
	}
	else if (heights_base.format() == gli::FORMAT_RGBA8_UNORM_PACK8)
	{
		const uint8_t *buffer = static_cast<const uint8_t *>(heights_base.data());
		for (int y = 0; y < height; y++)
			for (int x = 0; x < width; x++)
	...
	{
		LOGE("Unsupported heightmap format.\n");
		return 1;
	}
