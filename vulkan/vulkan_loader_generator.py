...
def entry_is_extension(entry):
    return entry.endswith('KHR') or \
        entry.endswith('KHX') or \
        entry.endswith('AMD') or \
        entry.endswith('NV') or \
        entry.endswith('GOOGLE') or \
        entry.endswith('NVX') or \
        entry.endswith('EXT') or \
        entry.endswith('NN') or \
        entry.endswith('MVK')
...
def main():
    ...
    for line in header:
        ...
        if m and (not entry_is_extension(m.group(1))) and m.group(2) != '(void)':
            entry = m.group(1)
        ...
        elif m and entry_is_extension(m.group(1)) and m.group(2) != '(void)':
            entry = m.group(1)
            if 'Android' in entry:
                continue
            if 'Xlib' in entry:
